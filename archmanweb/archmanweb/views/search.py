...
def search(request):
    ...
    selected_langs = request.GET.getlist("lang")
    selected_repos = request.GET.getlist("repo")
    ...
    exact_man = Case(When(name__iexact=term, then=Value(0)), default=Value(1), output_field=IntegerField())
    exact_sym = Case(When(from_name__iexact=term, then=Value(0)), default=Value(1), output_field=IntegerField())
    lang_prio = Case(When(lang="fa", then=Value(0)), When(lang="en", then=Value(1)), default=Value(2), output_field=IntegerField())
    repo_prio = Case(When(package__repo="core", then=Value(0)), When(package__repo="extra", then=Value(1)), default=Value(2), output_field=IntegerField())
    ...
    man_results = ManPage.objects.values("name", "section", "lang", "package__repo", "package__name") \
                                 ...
                                 .annotate(similarity=TrigramSimilarity("name", term), exact=exact_man, lang_p=lang_prio, repo_p=repo_prio) \
           .union(SymbolicLink.objects.values("from_name", "from_section", "lang", "package__repo", "package__name")
                                      ...
                                      .annotate(similarity=TrigramSimilarity("from_name", term), exact=exact_sym, lang_p=lang_prio, repo_p=repo_prio),
                  all=True) \
           .order_by("exact", "-similarity", "repo_p", "lang_p", "name", "section", "lang", "package__name")
    ...
    apropos_results = ManPage.objects.raw(f"""
            ...
            {apropos_filter}
            ORDER BY "rank" DESC,
                     CASE WHEN "{manpage_table}"."lang" = 'fa' THEN 0 WHEN "{manpage_table}"."lang" = 'en' THEN 1 ELSE 2 END ASC,
                     CASE WHEN "{package_table}"."repo" = 'core' THEN 0 WHEN "{package_table}"."repo" = 'extra' THEN 1 ELSE 2 END ASC,
                     "{manpage_table}"."name" ASC,
                     "{manpage_table}"."section" ASC,
                     "{manpage_table}"."lang" ASC,
                     "package__name" ASC,
                     "package__repo" ASC""",
    ...
    context = {
        ...
        "has_searched": True,
        "count_man": man_results.paginator.count,
        "count_apropos": apropos_results.paginator.count,
        "count_pkg": pkg_results.paginator.count,
        "selected_sections": selected_sections,
        "selected_langs": selected_langs,
        "selected_repos": selected_repos,
    }
