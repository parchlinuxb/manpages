...
from ..utils import reverse_man_url, search_url, extract_headings, _replace_section_heading_ids, postprocess
...
def _get_repo_key(repo):
    order = ("core", "extra", "multilib", "core-testing", "extra-testing", "multilib-testing")
    if repo in order:
...
def man_page(request, *, repo=None, pkgname=None, name_section_lang=None, url_output_type=None):
    ...
    if lang == "fa":
        converted_content = postprocess(converted_content, "html", lang)
    elif '<a class="permalink"' not in converted_content:
        converted_content = _replace_section_heading_ids(converted_content)
