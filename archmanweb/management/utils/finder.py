...
class ManPagesFinder:
                ...
                if info.islnk():
                    ...
                    if file.endswith(".gz"):
                        try:
                            ...
                            logger.error(f"File {file} in package {pkg.name}-{pkg.version}-{pkg.arch} is not a valid gzipped file.")
                            continue
                        file = file[:-3]
                    yield "file", file, man
    ...
    def pkg_exists(self, repo, pkgname):
        dbs = [db for db in self.sync_db.get_syncdbs() if db.name == repo]
        if not dbs:
            return False
        return dbs[0].get_pkg(pkgname) is not None
